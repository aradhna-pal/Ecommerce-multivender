...
function renderProducts(products){
    ...
    products.forEach(p=>{
        ...
        // IMAGE
        card.querySelector(".productImage").src =
            BASE + p.mainImage;

        // TEXT
        card.querySelector(".productName").innerText =
            p.brandName;

        card.querySelector(".name h5").innerText =
            p.name;

        card.querySelector(".product-details").innerText =
            p.shortDescription;
...
function openOptions(card,p){
    ...
    (p.colors || []).forEach(c=>{
        colors.innerHTML +=
        `<li><a style="background:${c.toLowerCase()}"></a></li>`;
    });
...
function closeSidebar(){
    document.querySelectorAll(".select-option-box")
...
// all code is correct but arrorw icon pr click krne pr niche wali gallerty ke img bhi scroll ko